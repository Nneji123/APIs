...
def predict_mask(im, confidence=85):
    ...
    image = image.resize(
        (200, 200)
    )  #  PIL image # could I upsample output instead? better?
    ...
    outputs = model(
        **encoding
    )  # odict with keys: ['logits', 'pred_boxes', 'pred_masks', 'last_hidden_state', 'encoder_last_hidden_state']
    logits = outputs.logits  # torch.Size([1, 100, 251]); class logits? but  why 251?
    bboxes = outputs.pred_boxes
    masks = (
        outputs.pred_masks
    )  # torch.Size([1, 100, 200, 200]); mask logits? for every pixel, score in each of the 100 classes? there is a mask per class

    # keep only the masks with high confidence?--------------------------------
    # compute the prob per mask (i.e., class), excluding the "no-object" class (the last one)
    prob_per_query = outputs.logits.softmax(-1)[..., :-1].max(-1)[
        0
    ]  # why logits last dim 251?
    # threshold the confidence
    keep = prob_per_query > confidence / 100.0

    # postprocess the mask (numpy arrays)
    label_per_pixel = (
        torch.argmax(masks[keep].squeeze(), dim=0).detach().numpy()
    )  # from the masks per class, select the highest per pixel
...
# predict_mask(im=np.array(Image.open('image.jpg')),confidence=85)
