...
async def generate_text(data: TextGeneration):
    query = simple_completion(prompt=data.prompt, length=data.max_length, temp=data.temperature, top_p=data.top_probability, top_k=data.top_k, rep=data.repetition)
    return query
...
async def advanced_generate_text(data: AdvancedTextGeneration):
    ...
    response = context_setting.completion(prompt=data.prompt,
                user=data.User,
                bot=data.Bot,
